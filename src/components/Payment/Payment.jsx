...
export const Payment = () => {
  ...
  const plitkaPrice = useSelector((state) => state.materialPage.priceMaterials)
    .filter(el => el.title === 'Плитка').map(el => el.price);

  const priceCement = priceMaterials.filter((el) => el.title === "Цемент").map(el => el.price);

  const priceSheben = priceMaterials.filter((el) => el.title === "Щебень").map(el => el.price);
  const pricePesok = priceMaterials.filter((el) => el.title === "Песок").map(el => el.price);
  const pricePorebrik = priceMaterials.filter(
    (el) => el.title === "Поребрик").map(el => el.price);
  ...
  const materialsCount = {
    plitka: parseInt(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Тротуарная плитка")
      ).map((el) => el.count)
    ),
    beton: parseInt(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бетон")
      ).map((el) => el.count)
    ),
    plitkaCement: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Тротуарная плитка")
      ).map((el) => el.count) / 10
    ),
    ...
    porebrik: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Поребрик")
      ).map((el) => el.count)
    ),
    bordur: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бордюр")
      ).map((el) => el.count)
    ),
    porebrikCement: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Поребрик")
      ).map((el) => el.count) / 14
    ),
    bordurCement: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бордюр")
      ).map((el) => el.count) / 14
    ),
    bordurPesok: Math.ceil(
      (useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бордюр")
      ).map((el) => el.count) /
        14) *
    ...
    betonCement: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бетон")
      ).map((el) => el.count) * 3.3
    ),
    betonPesok: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бетон")
      ).map((el) => el.count) * 0.864
    ),
    betonSheben: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бетон")
      ).map((el) => el.count) * 1.2
    ),
    porebrikPesok: Math.ceil(
      (useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Поребрик")
      ).map((el) => el.count) /
        14) *
    ...
    porebrikSheben: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Поребрик")
      ).map((el) => el.count) * 0.0142
    ),
    bordurSheben: Math.ceil(
      useSelector((state) =>
        state.paymentPage.cards.filter((el) => el.title === "Бордюр")
      ).map((el) => el.count) * 0.0142
    ),
  ...
  const materials = [
    ...
    priceCement !== ""
      ? createData(
        "Цемент (50кг)",
        priceCement,
        materialsCount.plitkaCement +
        materialsCount.porebrikCement +
        materialsCount.bordurCement +
    ...
    priceSheben !== ""
      ? createData(
        "Щебень",
        priceSheben,
        materialsCount.sheben +
        (checked && materialsCount.betonSheben) +
        materialsCount.porebrikSheben +
    ...
    pricePesok !== ""
      ? createData(
        "Песок",
        pricePesok,
        materialsCount.pesok +
        (checked && materialsCount.betonPesok) +
        materialsCount.porebrikPesok +
              ...
              <div className={style.topBar}>
                <div style={{ width: "300px" }}>
                  ...
                  <Button
                    className={
                      !isVisibleMaterials
                        ? style.buttonMaterials
                        : style.buttonMaterialsDelete
                    }
                    id="material"
                    ...
                    aria-expanded={open ? "true" : undefined}
                    onClick={
                      !isVisibleMaterials ? openMaterials : handleOpenMaterials
                    }
                  >
                    {isVisibleMaterials
                      ? "- Скрыть материалы"
                      ...
                      <AccordionDetails>
                        <div className={style.infoPanel}>
                          {plitkaPrice === "" &&
                            ...
                            priceSheben === "" &&
                            pricePorebrik === "" && (
                              <p className={style.infoMaterials}>
                                Заполните нужные вам поля
                              </p>
                            )}
                        </div>
                      ...
                      <div className={style.panelPlitkaCustom}>
                        <div className={style.panelPlitkaCustomInputs}>
                          ...
                          <TextField
                            ...
                            onChange={(e) => setCustomWorkCount(e.target.value)}
                            name="countS"
                            type="input"
                            label="Кол-во"
                          />
                          ...
                          <SelectUnits updateUnits={updateUnits} />
                        </div>
                      </div>
                      <div className={style.panelButtons}>
                        <Button variant="contained" onClick={addCustomWork}>
                          Добавить
                        </Button>
                        <Button
                          variant="text"
                ...
                (plitkaPrice > 0 ||
                  ...
                  >
                    <TableRow>
                      <TableCell
                        colSpan={4}
                        style={{ padding: "10px", color: "#967E76" }}
                      >
                          ...
                          elem.count > 0 && (
                            ...
                            >
                              ...
                              >
                                ...
                                {elem.name === "Поребрик"
                                  ? " м"
                                  : elem.name === "Тротуарная плитка"
                                    ? " м²"
                                    : elem.name === "Цемент (50кг)"
                                      ? ` шт`
                                      : " т"}
                              </TableCell>
                              <TableCell
                              ...
                              <TableCell
                                ...
                                align="right"
                              >
                                {parseInt(elem.count) * elem.price} ₽
                              </TableCell>
                            </TableRow>
                          )
                      )}
